...
class TweetsService {
    ...
    const hashtagDocuments = await Promise.all(
      hashtags.map((hashtag) => {
        ...
        return databaseService.hashtag.findOneAndUpdate(
          ...
          {
            upsert: true, //Cho phép chèn (insert) một tài liệu mới nếu không tìm thấy tài liệu nào khớp trong cơ sở dữ liệu.
            returnDocument: 'after' //Đảm bảo rằng tài liệu trả về là phiên bản sau khi được cập nhật hoặc chèn mới.
          }
        )
      })
    )
    return hashtagDocuments.map((hashtag) => (hashtag as WithId<Hashtag>)._id)
    // Ánh xạ qua các đối tượng hashtag và lấy ra _id của mỗi hashtag bằng cách ép kiểu về WithId<Hashtag>.
    //Cuối cùng, hàm trả về mảng _id của các hashtag đã được tìm thấy hoặc tạo mới.
  }
  ...
  async increaseView(tweet_id: string, user_id?: string) {
    ...
    const result = await databaseService.tweets.findOneAndUpdate(
      ...
      },
      {
        ...
        projection: {
          //hàm này dùng để lấy ra các trường bên dưới nếu để bằng 1 và loại bỏ hết mấy trường còn lại
          guest_views: 1,
          user_views: 1,
          updated_at: 1
        }
  ...
  async getNewFeeds({ user_id, limit, page }: { user_id: string; limit: number; page: number }) {
    ...
    const followed_user_ids = await databaseService.followers
      ...
      .toArray()
    const ids = followed_user_ids.map((item) => item.followed_user_ids) //danh sách id
